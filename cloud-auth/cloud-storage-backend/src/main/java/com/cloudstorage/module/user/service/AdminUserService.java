package com.cloudstorage.module.user.service;
...
public class AdminUserService {
    ...
    public String create(CreateUserRequest req) {
        if (userRepository.existsByUsername(req.getUsername())) {
            throw new ApiException(ResultCodes.BAD_REQUEST, "用户名已存在");
        }
    ...
    public void update(Long id, UpdateUserRequest req) {
        ...
        if (req.getQuotaBytes() != null && req.getQuotaBytes() > 0) {
            if (req.getQuotaBytes() < user.getUsedBytes()) {
                ...
                throw new ApiException(ResultCodes.QUOTA_TOO_LOW,
                        ResultCodes.QUOTA_TOO_LOW.getMessage() +
