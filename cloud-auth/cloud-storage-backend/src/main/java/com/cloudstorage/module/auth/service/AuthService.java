...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest req, HttpServletRequest http) {
        ...
        if (throttle.isRateLimited(ip)) {
            throw ApiException.of(ResultCodes.LOGIN_FAILED);
        }

        // 2. 锁定检查
        long remain = throttle.lockedRemainingMinutes(ip);
        if (remain > 0) {
            throw new ApiException(ResultCodes.ACCOUNT_LOCKED,
                    ResultCodes.ACCOUNT_LOCKED.getMessage() +
                            "（" + remain + " 分钟后可再试）");
        }
        ...
        if (user.isDisabled()) {
            ...
            throw ApiException.of(ResultCodes.ACCOUNT_DISABLED);
        }
    ...
    public LoginResponse refresh(RefreshRequest req) {
        ...
        try {
            ...
            throw ApiException.of(ResultCodes.TOKEN_INVALID);
        }
        ...
        User user = userRepository.findById(userId).orElseThrow(() -> ApiException.of(ResultCodes.TOKEN_INVALID));
        if (user.isDisabled()) {
            throw ApiException.of(ResultCodes.ACCOUNT_DISABLED);
        }
    ...
    public void changePassword(CurrentUser cu, ChangePasswordRequest req) {
        User user = userRepository.findById(cu.getId())
                .orElseThrow(() -> ApiException.of(ResultCodes.USER_NOT_FOUND));
