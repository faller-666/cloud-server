 ...
 *
 * 用原生 SQL 直接操作 users 表（表 Owner 为 A 组，B 组读写 used_bytes/quota_bytes，
 * 只读 extra_bytes——计费增量额度，由 D 组维护），不引入完整 UserEntity，解耦 A 组表结构。
...
public class UserQuotaRepository {
    ...
    /** 预检：只读判断会不会超配额（供 init 快速拒绝，不加锁）。总额度 = 免费额度 + 增量额度。 */
    public boolean checkAvailable(long userId, long delta) {
        try {
            Long remaining = (Long) em.createNativeQuery(
                            "SELECT quota_bytes + extra_bytes - used_bytes FROM users WHERE id = :id")
    ...
    /**
     * 实扣配额（complete 时）——条件 UPDATE 原子防超卖。总额度 = 免费额度 + 增量额度。
    ...
    public boolean tryReserve(long userId, long delta) {
        int updated = em.createNativeQuery(
                        ...
                        "WHERE id = :id AND status = 'active' AND used_bytes + :delta <= quota_bytes + extra_bytes")
